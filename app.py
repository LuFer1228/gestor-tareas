...
@login_required
def dashboard():
    tareas_pendientes = Tarea.query.filter_by(usuario_id=current_user.id, completada=False).order_by(Tarea.fecha_limite.asc()).all()
    tareas_completadas = Tarea.query.filter_by(usuario_id=current_user.id, completada=True).all()
    hoy = date.today()
...
def nueva_tarea():
    if request.method == 'POST':
        ...
        tarea = Tarea(titulo=titulo, descripcion=descripcion, fecha_limite=fecha_limite, usuario_id=current_user.id)
...
def nuevo_evento():
    ...
    evento = Evento(titulo=titulo, fecha=fecha, hora=hora, usuario_id=current_user.id)
...
def chatbot():
    ...
    contexto_completadas = "".join([f"- {t.titulo}\n" for t in tareas_completadas])
    contexto_eventos = "".join([f"- {e.titulo} ({e.fecha.strftime('%d/%m/%Y')}{' a las ' + e.hora if e.hora else ''})\n" for e in eventos])
    system_prompt = f"""Eres un asistente academico personal amigable para {current_user.nombre}.
...
{contexto_eventos if contexto_eventos else 'No tiene eventos programados.'}"""
    ...
    try:
        respuesta = groq_client.chat.completions.create(model="llama-3.3-70b-versatile", messages=messages)
...
def generar_plan(tarea_id):
    ...
    try:
        respuesta = groq_client.chat.completions.create(
            ...
            messages=[
                {"role": "system", "content": "Genera exactamente 5 subtareas concretas para completar la tarea del estudiante. Responde SOLO con las 5 subtareas, una por linea, sin numeros ni vinetas, sin texto extra."},
                {"role": "user", "content": f"Titulo: {tarea.titulo}\nDescripcion: {tarea.descripcion or 'Sin descripcion'}\nFecha limite: {tarea.fecha_limite}"}
            ]
        )
        ...
        for titulo in subtareas_texto:
            ...
            db.session.add(subtarea)
        db.session.commit()
        flash(f'La IA genero {len(subtareas_texto)} subtareas!', 'success')
    except Exception as e:
...
def resumir_tarea(tarea_id):
    ...
    if not tarea.descripcion:
        flash('La tarea no tiene descripcion para resumir.', 'warning')
        return redirect(url_for('dashboard'))
    try:
        ...
        respuesta = groq_client.chat.completions.create(
            model="llama-3.3-70b-versatile",
            messages=[
                {"role": "system", "content": "Resume la descripcion de la tarea academica en maximo 2 lineas, de forma clara y directa. Solo devuelve el resumen, sin texto extra."},
                {"role": "user", "content": tarea.descripcion}
            ]
        )
        tarea.descripcion = respuesta.choices[0].message.content.strip()
        db.session.commit()
        flash('Descripcion resumida por la IA!', 'success')
    except Exception as e:
        flash(f'Error: {str(e)}', 'danger')
    return redirect(url_for('dashboard'))
...
def restaurar_descripcion(tarea_id):
    ...
    if tarea.descripcion_original:
        ...
        db.session.commit()
        flash('Descripcion restaurada!', 'success')
    else:
        flash('No hay descripcion original guardada.', 'warning')
    return redirect(url_for('dashboard'))
